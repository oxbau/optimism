...
func main() {
	...
	if err := app.Run(os.Args); err != nil {
		log.Crit("Error staring CLI app", "error", err.Error())
	}
}
...
func generateBindings(c *cli.Context) error {
	...
	case "all":
		...
		if err := localBindingsGenerator.generateBindings(); err != nil {
			log.Crit("Error generating local bindings", "error", err.Error())
		}
		...
		if err := remoteBindingsGenerator.generateBindings(); err != nil {
			log.Crit("Error generating remote bindings", "error", err.Error())
		}
