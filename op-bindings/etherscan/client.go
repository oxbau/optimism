...
func (c *client) FetchAbi(address string) (string, error) {
	url := c.constructUrl(c.baseUrl, "getabi", address, "contract", fmt.Sprintf("address=%s", address))
...
func (c *client) FetchDeployedBytecode(address string) (string, error) {
	url := c.constructUrl(c.baseUrl, "eth_getCode", address, "proxy", fmt.Sprintf("address=%s", address))
...
func (c *client) FetchDeploymentTxHash(address string) (string, error) {
	url := c.constructUrl(c.baseUrl, "getcontractcreation", address, "contract", fmt.Sprintf("contractaddresses=%s", address))
	response, err := c.fetchEtherscanApi(url)
	if err != nil {
		return "", err
	}
...
func (c *client) FetchDeploymentTx(txHash string) (TxInfo, error) {
	url := c.constructUrl(c.baseUrl, "eth_getTransactionByHash", txHash, "proxy", fmt.Sprintf("txHash=%s&tag=latest", txHash))
	response, err := c.fetchEtherscanRpc(url)
	if err != nil {
		return TxInfo{}, err
	}
...
func (c *client) constructUrl(baseUrl, action, address, module, params string) string {
